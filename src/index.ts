...
register("gsc_query_overlap", "Find queries where multiple site pages earn impressions, useful for consolidation or intent differentiation decisions.", { site_url: siteUrl, days, min_impressions: z.number().int().min(1).default(50), min_pages: z.number().int().min(2).default(2), limit }, (i: any) => queryPageOverlap({ siteUrl: i.site_url, days: i.days, minImpressions: i.min_impressions, minPages: i.min_pages, limit: i.limit }));
