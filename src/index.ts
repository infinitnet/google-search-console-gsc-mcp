...
register("gsc_query_overlap", "Find two-page query and impression overlap, scoring cannibalization severity with balanced impressions so one-sided overlaps are discounted.", { site_url: siteUrl, days, min_impressions: z.number().int().min(1).default(50), min_pages: z.number().int().min(2).default(2), limit }, (i: any) => queryPageOverlap({ siteUrl: i.site_url, days: i.days, minImpressions: i.min_impressions, minPages: i.min_pages, limit: i.limit }));
