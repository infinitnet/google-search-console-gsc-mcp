...
const server = new McpServer({ name: "infinitnet-google-search-console-gsc-mcp-server", version: "1.0.1" });
...
register("gsc_multi_property_health", "Compare health summaries across up to 20 Search Console properties configured for one account or agency workflow.", { site_urls: z.array(z.string().min(1)).min(1).max(20).optional(), days }, (i: any) => multiPropertyOverview({ siteUrls: i.site_urls, days: i.days }));
